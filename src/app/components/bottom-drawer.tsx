...
export function BottomDrawer({ isOpen, onClose, onSelect, onActivateVertical }: BottomDrawerProps) {
  ...
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="bottom" className="h-[85vh] p-0 flex flex-col bg-stone-50 rounded-t-3xl">
        <SheetHeader className="p-6 border-b border-stone-200 bg-white/80 backdrop-blur-sm rounded-t-3xl shrink-0">
          <div className="flex items-center justify-between mb-4">
             <SheetTitle className="text-2xl font-bold text-stone-900 tracking-tight flex items-center gap-2">
                <Compass className="w-6 h-6 text-stone-900" /> Discovery Engine
             </SheetTitle>
             <SheetDescription className="text-stone-500 text-xs">Explore apps, actions, and city services</SheetDescription>
          </div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="p-6">
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        isActive 
                          ...
                          : `${config.bg} ${config.color} ${config.border} hover:brightness-95`
                      )}
                    >
            ...
            {/* Actions Grid (Nano Chips) */}
            <div className="flex flex-wrap content-start gap-2 pb-20">
              <AnimatePresence mode="popLayout">
                {filteredActions.map((action) => (
                  <motion.button
                    ...
                    className="group flex items-center gap-2 pl-1 pr-3 py-1 bg-white border border-stone-200 rounded-full cursor-pointer hover:border-stone-300 hover:shadow-sm transition-all"
                  >
                    <div className={cn("w-6 h-6 rounded-full flex items-center justify-center shrink-0", CATEGORY_THEMES[action.category].bg)}>
                      <action.icon className={cn("w-3.5 h-3.5", CATEGORY_THEMES[action.category].color)} />
                    </div>
                    <span className="font-medium text-stone-700 text-xs group-hover:text-stone-900">{action.title}</span>
                  </motion.button>
                ))}
              </AnimatePresence>
            </div>
